...
fn main() {
    ...
    match fork() {
        ...
        Ok(ForkResult::Child) => {
            //closing read end of the pipe, child will only write output to pw
            close(pr).expect("[main] close(pw) failed");

            //child is closing its STDIN, no inputs
            let dev_null_r = open("/dev/null", OFlag::O_RDONLY, Mode::empty()).unwrap();
            dup2(dev_null_r, 0).expect("dup2(STDIN) failed");
            // child has redirected its STDOUT and STDERR to write end of pipe
    ...
    // In parent, close write end, It will only read
